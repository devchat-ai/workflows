...
def document_symbols(abspath: str):
    code = (
        ...
        "return await vscode.commands.executeCommand('vscode.executeDocumentSymbolProvider', fileUri);"
    )
...
def workspace_symbols(query: str):
    code = (
        f"return await vscode.commands.executeCommand('vscode.executeWorkspaceSymbolProvider', '{query}');"
    )
